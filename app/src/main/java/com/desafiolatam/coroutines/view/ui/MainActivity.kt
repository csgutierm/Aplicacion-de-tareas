...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val fabDeleteAllTask: FloatingActionButton = findViewById(R.id.fabDeleteAllTask)
        fabDeleteAllTask.setOnClickListener {
            lifecycleScope.launch {
                taskDao.deleteAllTasks()
                Toast.makeText(
                    this@MainActivity,
                    "Todas las tareas se han eliminado con éxito",
                    Toast.LENGTH_LONG
                ).show()
            }
        }
...
/*        val checkbox = findViewById<CheckBox>(R.id.checkbox)
    ...
    private fun initRecyclerView(taskList: List<TaskEntity>) {
        ...
        adapter.onLongClick = {
            ...
            intent.putExtra("taskTitle", it.title)
            intent.putExtra("taskDescription", it.description)
            intent.putExtra("taskCompletada", it.completada)
            startActivity(intent)
//            deleteTask(it)
