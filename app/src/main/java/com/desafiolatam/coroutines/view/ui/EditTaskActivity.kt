...
class EditTaskActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val taskCompletada = intent.getBooleanExtra("taskCompletada", false)

        // Utiliza los datos como desees, por ejemplo, mostrarlos en vistas
        editDescripcion.setText(taskDescription)
        editTitulo.setText(taskTitle)

        button.setOnClickListener  {

            val updatedTitle = editTitulo.text.toString()
            val updatedDescription = editDescripcion.text.toString()

            val task = TaskEntity(
                id = taskId,
                title = updatedTitle,
                description = updatedDescription,
                completada = taskCompletada
            )
...
/*           val task  = TaskEntity(
            ...
            lifecycleScope.launch {
                ...
                Toast.makeText(this@EditTaskActivity, "Tarea editada con éxito", Toast.LENGTH_SHORT).show()
